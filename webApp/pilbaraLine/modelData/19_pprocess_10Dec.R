#Manoves rows from the waterpoints dataset based on management decisions
modify.plb <- function(plb, future.irrigation, current.irrigation, IPA, Properties, Name, Dwelling){
	...
	#IPA areas
	if (IPA == TRUE) {
	plb <- subset(plb,!(plb$Property == "Frazier Downs" & IPA==0))
	#plb <- subset(plb,!(plb$Property == "Frazier Downs"))
	#plb <- subset(plb,!(plb$IPA==1))
	plb$FID <- c(1:nrow(plb))
	rownames(plb) <- plb$FID }
	#Properties
	if (Properties == TRUE) {
	plb <- subset(plb,!plb$Property== "Wallal Downs")
	#plb <- subset(plb,!plb$Property == "Nita Downs")
	plb$FID <- c(1:nrow(plb))
	rownames(plb) <- plb$FID }
	#Dwellings
	if (Dwelling == FALSE) {
...
#Function called by spread function to set unique lambda values for irrigation points
unique.lambda <- function(occp,delta){
	irr1 <- occp[,1] == which(plb$Status == "Current" & plb$Property == "Shamrock Gardens")
	occp[,10][irr1] <- delta*0.03674617*3.1233
...
#x <- seq(1,25000,1)
#x <- 5001
#test <- (x*u^v*v*sqrt(v^v*(u^2*v+x^2)^(-2-v))) #1D dispersal kernel
#plot(test)

#trunc.50 <- matrix(NA,nrow(fits),1)
#for (i in 1:nrow(fits)){
#maxd <- 50000
#u <- fits[i,1]
#v <- fits[i,2]
#integrand <- function(x) {(x*u^v*v*sqrt(v^v*(u^2*v+x^2)^(-2-v)))}
#tr <- integrate(integrand, lower = 0, upper = maxd)
#tr <- tr$value
#trunc.50[i,1] <- tr
#}
#colnames(trunc.50) <- "50km"

#trunc.65 <- matrix(NA,nrow(fits),1)
#for (i in 1:nrow(fits)){
#maxd <- 65000
#u <- fits[i,1]
#v <- fits[i,2]
...
#trunc.65[i,1] <- tr
#}
#colnames(trunc.65) <- "65km"

#trunc.100 <- matrix(NA,nrow(fits),1)
#for (i in 1:nrow(fits)){
#maxd <- 100000
#u <- fits[i,1]
#v <- fits[i,2]
...
spread.pilb.failure<-function(pop, gens, pairs, target, delta, r, trunc.dist){ #pairs is a list from pdist.fast  
	for (i in 1:gens){ #loop through time
		...
		thresh1<-recruits[recruits[,"recruit.ID"] == which(plb$Status == "Current" & plb$Property == "Shalemar")][1]
		thresh2<-recruits[recruits[,"recruit.ID"] == which(plb$Status == "Current" & plb$Property == "Shamrock Gardens")][1]
		thresh3<-recruits[recruits[,"recruit.ID"] == which(plb$Status == "Current" & plb$Property == "Pardoo")][1]
		thresh4<-recruits[recruits[,"recruit.ID"] == which(plb$Status == "Current" & plb$Property == "Wallal Downs")][1]
		thresh5<-recruits[recruits[,"recruit.ID"] == which(plb$Name == "La Grange")][1]
	
		ifelse(recruits[thresh1,2] < 2, recruits[thresh1,2] <- 0, recruits[thresh1,2] <- recruits[thresh1,2]) 
		ifelse(recruits[thresh2,2] < 2, recruits[thresh2,2] <- 0, recruits[thresh2,2] <- recruits[thresh2,2]) 
		ifelse(recruits[thresh3,2] < 2, recruits[thresh3,2] <- 0, recruits[thresh3,2] <- recruits[thresh3,2]) 
		ifelse(recruits[thresh4,2] < 2, recruits[thresh4,2] <- 0, recruits[thresh4,2] <- recruits[thresh4,2]) 
		ifelse(recruits[thresh5,2] < 2, recruits[thresh5,2] <- 0, recruits[thresh5,2] <- recruits[thresh5,2]) 
...
#This function Manoves waterbodies at a site until a fixed budget is spent
...
cost.waterpoint2 <- function(plb,gens,dst,plot.cost) { ##Include discounting for maintanaence and replacement -> C = P(1+r)^t
cost.WP <- matrix(NA,nrow(plb),1)
purpose <- plb$Purpose
type <- plb$Type
...
for (cc in 1:nrow(plb)){
	...
	{cost.WP[cc] <- sum(Area[cc]*100*30*65*(1+0.025)^disc)} #Convert km2 to ha and multiply by profit gained for hay per ha 
